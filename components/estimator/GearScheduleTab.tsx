...
export function GearScheduleTab() {
  ...
  const items = state.gearSchedule;
  ...
  function handlePriceBlur(id: string, raw: string) {
    ...
    updateGearItem(id, { quotedPrice, quoteStatus });
  }

  function handleMarkupBlur(id: string, raw: string) {
    const pct    = parseFloat(raw);
    const markup = isNaN(pct) ? 0.05 : Math.max(0, pct) / 100;
    updateGearItem(id, { markup });
  }

  function handleTagBlur(id: string, value: string) {
    updateGearItem(id, { typeLabel: value });
  }

  function handleDescBlur(id: string, value: string) {
    updateGearItem(id, { description: value });
  }
  ...
  async function handleSendQuotes() {
    ...
    try {
      // gear-quote API expects { tag, description, qty }
      ...
      items
        ...
        .forEach(i => updateGearItem(i.id, { quoteStatus: 'QUOTED' }));
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="text-lg font-bold text-[#1e3a8a]">Gear Schedule</h2>
      ...
      {showAddForm && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
          <h3 className="text-sm font-bold text-[#1e3a8a] mb-3">Add Gear Item</h3>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              min={0}
              step={0.01}
              value={state.gearTotalQuote ?? ''}
              onChange={e => setState(s => ({
                ...s,
                gearTotalQuote: parseFloat(e.target.value) || null,
              }))}
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              min={0}
              max={50}
              value={Math.round(state.gearQuoteMarkup * 100)}
              onChange={e => setState(s => ({
                ...s,
                gearQuoteMarkup: (parseInt(e.target.value) || 0) / 100,
              }))}
              className="border border-gray-300 rounded px-3 py-2 text-sm w-20 text-center focus:outline-none focus:border-blue-400"
            />
            <span className="text-gray-500 text-sm">%</span>
          ...
          {state.gearTotalQuote ? (
            <div className="text-lg font-bold text-[#1e3a8a]">
              {fmt$(state.gearTotalQuote * (1 + state.gearQuoteMarkup))}
