...
export function BOMReferenceTab() {
  const { priceOverrides } = useEstimatorContext();
  const R = getRates();
  ...
  const categories = useMemo(() =>
    ['All Categories', ...Array.from(new Set(BOM.map(b => b.cat)))],
  []);
  ...
  }, [search, catFilter, unitFilter, gcOnly]);
  ...
  // Markup % display (reads live rates)
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {pageItems.map((item, i) => {
                ...
                const displayMat  = overridden ? (priceOverrides[item.id] as number) : item.mat;
                ...
                return (
                  <tr key={item.id} className={i % 2 === 0 ? '' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-1.5 text-right font-mono font-medium text-[#1a3a5c]">
                      {fmt$(applyMarkup(displayMat, item.mk))}
                    </td>
                    <td className="px-3 py-1.5 text-right font-mono text-gray-500">
