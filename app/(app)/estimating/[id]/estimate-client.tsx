...
import {
  calcBid, calcLine, calcConduitRun, calcMcHomeRun, calcThreeWay, fmt$, adjustLhr,
} from "@/lib/estimating";
import type {
...
function AssembliesTab({
  assemblies, setAssemblies, data,
}: {
  assemblies: Assembly[];
  setAssemblies: (a: Assembly[]) => void;
  data: EstimateData;
}) {
  const [showForm, setShowForm] = useState<AssemblyType | null>(null);

  function addAssembly(asm: Assembly) {
    setAssemblies([...assemblies, asm]);
    setShowForm(null);
  }
  ...
  const asmTotals = assemblies.reduce((acc, asm) => {
    let r = { mat: 0, lhr: 0 };
    if (asm.type === "CONDUIT_RUN") r = calcConduitRun(asm.params as Parameters<typeof calcConduitRun>[0], data);
    else if (asm.type === "MC_HOME_RUN") r = calcMcHomeRun(asm.params as Parameters<typeof calcMcHomeRun>[0], data);
    else if (asm.type === "THREE_WAY") r = calcThreeWay(asm.params as Parameters<typeof calcThreeWay>[0], data);
    else if (asm.type === "CUSTOM") {
      r = { mat: Number(asm.params.mat) || 0, lhr: Number(asm.params.lhr) || 0 };
    }
    return { mat: acc.mat + r.mat, lhr: acc.lhr + r.lhr, labor: acc.lhr + r.lhr * data.laborRate, total: acc.total + r.mat + r.lhr * data.laborRate };
  }, { mat: 0, lhr: 0, labor: 0, total: 0 });

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-2">
        {(["CONDUIT_RUN", "MC_HOME_RUN", "THREE_WAY", "CUSTOM"] as AssemblyType[]).map(type => (
          <button
            key={type}
            onClick={() => setShowForm(type)}
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium border border-[#002D72]/30 text-[#002D72] rounded-lg hover:bg-blue-50 transition-colors"
          >
            <Plus className="w-3.5 h-3.5" />
            {type === "CONDUIT_RUN" ? "Conduit Run" : type === "MC_HOME_RUN" ? "MC Home Run" : type === "THREE_WAY" ? "3-Way Circuit" : "Custom Assembly"}
          </button>
        ))}
      </div>
      ...
      {assemblies.length > 0 && (
        <div className="space-y-3">
          {assemblies.map(asm => {
            ...
            const laborCost = r.lhr * data.laborRate;
            return (
              <div key={asm.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="font-medium text-gray-900 truncate">{asm.label}</div>
                  <div className="text-xs text-gray-400 mt-0.5">{asm.type.replace("_", " ")}</div>
                  <div className="flex gap-4 mt-2 text-xs font-mono text-gray-600">
                    <span>Mat: {fmt$(r.mat)}</span>
                    <span>Hrs: {r.lhr.toFixed(2)}</span>
                    <span>Labor: {fmt$(laborCost)}</span>
                    <span className="font-bold text-[#002D72]">Total: {fmt$(r.mat + laborCost)}</span>
                  </div>
                </div>
                <button onClick={() => removeAssembly(asm.id)} className="p-1.5 text-gray-300 hover:text-red-500 transition-colors shrink-0">
          ...
          <div className="p-3 bg-blue-50 rounded-xl text-sm font-semibold text-[#002D72] flex justify-between">
            <span>Assembly Section Totals</span>
            <span className="font-mono">{fmt$(asmTotals.mat + asmTotals.lhr * data.laborRate)}</span>
          </div>
        </div>
      )}
    </div>
...
function AssemblyForm({ type, data, onAdd, onCancel }: {
  type: AssemblyType;
  data: EstimateData;
  onAdd: (asm: Assembly) => void;
  onCancel: () => void;
}) {
  // Conduit Run state
  const [crSize, setCrSize] = useState("3/4");
  const [crFootage, setCrFootage] = useState(50);
  const [crConductors, setCrConductors] = useState(2);
  const [crWireSize, setCrWireSize] = useState("12");
  const [crDifficulty, setCrDifficulty] = useState(1.0);

  // MC Home Run state
  const [mcWireSize, setMcWireSize] = useState<"12" | "10" | "8" | "6">("12");
  const [mcFootage, setMcFootage] = useState(50);
  const [mcCircuits, setMcCircuits] = useState(1);
  const [mcHasBox, setMcHasBox] = useState(false);

  // 3-Way state
  const [twFeed, setTwFeed] = useState(25);
  const [twTraveler, setTwTraveler] = useState(15);
  const [twSwitchLeg, setTwSwitchLeg] = useState(10);
  const [twWireSize, setTwWireSize] = useState<"12" | "10">("12");

  // Custom state
  const [custLabel, setCustLabel] = useState("");
  const [custMat, setCustMat] = useState(0);
  const [custLhr, setCustLhr] = useState(0);

  function getPreview() {
    if (type === "CONDUIT_RUN") return calcConduitRun({ size: crSize, footage: crFootage, conductors: crConductors, wireSize: crWireSize, difficulty: crDifficulty }, data);
    if (type === "MC_HOME_RUN") return calcMcHomeRun({ wireSize: mcWireSize, footage: mcFootage, circuits: mcCircuits, hasBox: mcHasBox }, data);
    if (type === "THREE_WAY") return calcThreeWay({ feedFt: twFeed, travelerFt: twTraveler, switchLegFt: twSwitchLeg, wireSize: twWireSize }, data);
    return { mat: custMat, lhr: custLhr };
  }

  function buildAssembly(): Assembly {
    if (type === "CONDUIT_RUN") {
      return { id: newId(), type, label: `${crSize}" EMT — ${crFootage}ft — ${crConductors}x #${crWireSize}`, params: { size: crSize, footage: crFootage, conductors: crConductors, wireSize: crWireSize, difficulty: crDifficulty } };
    }
    if (type === "MC_HOME_RUN") {
      return { id: newId(), type, label: `${mcWireSize}/2 MC — ${mcCircuits} circuit${mcCircuits > 1 ? "s" : ""} — ${mcFootage}ft each`, params: { wireSize: mcWireSize, footage: mcFootage, circuits: mcCircuits, hasBox: mcHasBox } };
    }
    if (type === "THREE_WAY") {
      return { id: newId(), type, label: `3-Way — ${twFeed}ft feed / ${twTraveler}ft travelers / ${twSwitchLeg}ft switch leg`, params: { feedFt: twFeed, travelerFt: twTraveler, switchLegFt: twSwitchLeg, wireSize: twWireSize } };
    }
    return { id: newId(), type: "CUSTOM", label: custLabel || "Custom Assembly", params: { mat: custMat, lhr: custLhr } };
  }

  const preview = getPreview();
  const laborCost = preview.lhr * data.laborRate;

  return (
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-[#002D72]">
          {type === "CONDUIT_RUN" ? "Conduit Run" : type === "MC_HOME_RUN" ? "MC Home Run" : type === "THREE_WAY" ? "3-Way Circuit" : "Custom Assembly"}
        </h3>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {type === "CONDUIT_RUN" && (
          <>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Size</label>
              <select value={crSize} onChange={e => setCrSize(e.target.value)} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]">
                {["3/4", "1", "1-1/2", "2", "3", "4"].map(s => <option key={s} value={s}>{s}"</option>)}
              </select>
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Footage</label>
              <input type="number" value={crFootage} min={1} onChange={e => setCrFootage(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Conductors</label>
              <input type="number" value={crConductors} min={1} max={6} onChange={e => setCrConductors(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Wire Size</label>
              <select value={crWireSize} onChange={e => setCrWireSize(e.target.value)} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]">
                {["12", "10", "8", "6"].map(s => <option key={s} value={s}>#{s}</option>)}
              </select>
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Difficulty</label>
              <select value={crDifficulty} onChange={e => setCrDifficulty(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]">
                {[0.8, 1.0, 1.2, 1.5].map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
          </>
        )}
        {type === "MC_HOME_RUN" && (
          <>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Wire Size</label>
              <select value={mcWireSize} onChange={e => setMcWireSize(e.target.value as "12" | "10" | "8" | "6")} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]">
                {["12", "10", "8", "6"].map(s => <option key={s} value={s}>#{s}</option>)}
              </select>
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Footage/Circuit</label>
              <input type="number" value={mcFootage} min={1} onChange={e => setMcFootage(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Circuits</label>
              <input type="number" value={mcCircuits} min={1} max={20} onChange={e => setMcCircuits(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div className="flex items-end">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={mcHasBox} onChange={e => setMcHasBox(e.target.checked)} className="w-4 h-4 rounded" />
                <span>Box at end</span>
              </label>
            </div>
          </>
        )}
        {type === "THREE_WAY" && (
          <>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Feed (ft)</label>
              <input type="number" value={twFeed} min={1} onChange={e => setTwFeed(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Travelers (ft)</label>
              <input type="number" value={twTraveler} min={1} onChange={e => setTwTraveler(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Switch Leg (ft)</label>
              <input type="number" value={twSwitchLeg} min={1} onChange={e => setTwSwitchLeg(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Wire Size</label>
              <select value={twWireSize} onChange={e => setTwWireSize(e.target.value as "12" | "10")} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]">
                {["12", "10"].map(s => <option key={s} value={s}>#{s}</option>)}
              </select>
            </div>
          </>
        )}
        {type === "CUSTOM" && (
          <>
            <div className="col-span-2 sm:col-span-3">
              ...
              <input type="text" value={custLabel} onChange={e => setCustLabel(e.target.value)} placeholder="Assembly description" className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Material Cost ($)</label>
              <input type="number" value={custMat} min={0} step={0.01} onChange={e => setCustMat(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
            <div><label className="block text-xs font-medium text-gray-600 mb-1">Labor Hours</label>
              <input type="number" value={custLhr} min={0} step={0.01} onChange={e => setCustLhr(Number(e.target.value))} className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#002D72]" />
            </div>
          </>
        )}
      </div>
      ...
      {/* Preview */}
      <div className="bg-white rounded-lg p-3 text-sm flex gap-4 flex-wrap font-mono">
        <span>Mat: <strong>{fmt$(preview.mat)}</strong></span>
        <span>Hrs: <strong>{preview.lhr.toFixed(2)}</strong></span>
        <span>Labor: <strong>{fmt$(laborCost)}</strong></span>
        <span>Total: <strong className="text-[#002D72]">{fmt$(preview.mat + laborCost)}</strong></span>
      </div>

      <div className="flex justify-end gap-2">
        <button onClick={onCancel} className="text-sm text-gray-500 hover:text-gray-700 px-3 py-1.5">Cancel</button>
        <button onClick={() => onAdd(buildAssembly())} className="bg-[#002D72] text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-[#003d99] transition-colors">
          Add Assembly
        </button>
      </div>
    </div>
