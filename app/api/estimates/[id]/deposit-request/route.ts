...
export async function POST(
  ...
  const estimate = await prisma.estimate.findUnique({
    where: { id: estimateId },
    select: { name: true, estimateNumber: true, clientName: true, address: true },
  });
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(Text, { style: DS.value } as Record<string, unknown>,
          [estimate.clientName, estimate.address].filter(Boolean).join("\n") || "—"
        )
      ),
      React.createElement(
        View,
        ...
        React.createElement(Text, { style: DS.value } as Record<string, unknown>,
          `${estimate.name}  (Est. #${estimate.estimateNumber})`
        ),
  ...
  const filename = `DepositRequest_${estimate.estimateNumber}.pdf`;
