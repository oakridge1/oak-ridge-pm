  ...
  try {
    const info = await transport.sendMail({
      ...
      text: [
        `Hi ${displayName},`,
        "",
        `Your ${COMPANY_DBA} project management account has been activated.`,
        "",
      ...
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;color:#1a1a1a">
          <div style="margin-bottom:24px">
            ...
            <h1 style="font-size:22px;font-weight:700;color:${BRAND_BLUE};margin:6px 0 0">Your account is active</h1>
          </div>
          <p style="margin:0 0 16px">Hi <strong>${displayName}</strong>,</p>
          <p style="margin:0 0 24px">Your ${COMPANY_DBA} project management account has been activated. You can now sign in using your Google account.</p>
          <a href="${loginUrl}"
             style="display:inline-block;background:${BRAND_BLUE};color:#fff;font-weight:600;font-size:14px;padding:12px 24px;border-radius:8px;text-decoration:none">
            Sign In →
          </a>
          ...
          <p style="font-size:11px;color:#aaa;margin:0">${COMPANY_NAME} — ${APP_URL}</p>
        </div>
      `,
    });
